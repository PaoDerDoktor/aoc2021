...
def day18_part2_main() -> int:
    with open("day 18/inputs.txt", 'r') as inFile:
        numList = [load(_.strip()) for _ in inFile.readlines()]
        
        maxMagnitude: int = -1
        for firstOperand in numList:
            secondNumList = deepcopy(numList)
            secondNumList.pop(secondNumList.index(firstOperand))
            for secondOperand in secondNumList:
                node:      BinNode = list_to_tree(firstOperand)
                nodeToAdd: BinNode = list_to_tree(secondOperand)
                
                node = list_to_tree(load(f"[{repr(node)}, {repr(nodeToAdd)}]"))
                node.reduce()
                
                magnitude = node.get_magnitude()
